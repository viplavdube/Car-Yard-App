...
from oauthlib.common import extract_params, safe_string_equals, unicode_type, urldecode
...
def verify_hmac_sha1(request, client_secret=None, resource_owner_secret=None):
    ...
    signature = sign_hmac_sha1(sig_base_str, client_secret, resource_owner_secret)
    match = safe_string_equals(signature, request.signature)
    if not match:
        log.debug("Verify HMAC-SHA1 failed: signature base string: %s", sig_base_str)
    return match
...
def verify_hmac_sha256(request, client_secret=None, resource_owner_secret=None):
    ...
    bs_uri = base_string_uri(request.uri)
    sig_base_str = signature_base_string(request.http_method, bs_uri, norm_params)
    signature = sign_hmac_sha256(sig_base_str, client_secret, resource_owner_secret)
    match = safe_string_equals(signature, request.signature)
    if not match:
        log.debug("Verify HMAC-SHA256 failed: signature base string: %s", sig_base_str)
    return match
...
def verify_rsa_sha1(request, rsa_public_key):
    ...
    if not verify_ok:
        log.debug("Verify RSA-SHA1 failed: signature base string: %s", sig_base_str)
    return verify_ok
